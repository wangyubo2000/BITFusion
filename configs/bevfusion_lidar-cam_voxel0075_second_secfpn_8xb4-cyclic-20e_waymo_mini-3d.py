_base_ = [
    './bevfusion_lidar_voxel0075_second_secfpn_8xb4-cyclic-20e_waymo_mini-3d.py'
]
...
model = dict(
    ...
    cam_guided_plugin=dict(
        ...
        enable_fp16=True,
        heads=2, dim_head=24),#2）看一下我投影的过程对不对
   lidar_guided_plugin=dict(
        type='LidarGuidedImgGating',
        c_img=64, c_lidar=256,
        beta=0.5,
    ...
    fusion_layer=dict(
        type='ConvFuser', in_channels=[64, 256], out_channels=256)) #[80,256]->[64.256]

train_pipeline = [
    dict(type='NormalizeCam2ImgToK'),
    dict(
        type='AttachCamFolder',
        cam_prefixes=cam_prefixes,
        base_dir='/home/wangyubo/data/Waymo_mini/kitti_format/training'
    ),
    dict(
